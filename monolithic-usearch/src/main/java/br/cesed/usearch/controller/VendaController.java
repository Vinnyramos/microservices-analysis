...
public class VendaController {
	...
	public String getVenda(@PathVariable String id) {
		...
		Venda venda = vendaService.getById(id);
		Pedido pedido = restTemplate1.getForObject("http://localhost:9090/pedido/"+venda.getIdPedido(), Pedido.class);
		Produto produto = restTemplate2.getForObject("http://localhost:9090/produto/"+pedido.getIdProduto(), Produto.class);
		Usuario usuario = restTemplate3.getForObject("http://localhost:9090/usuario/"+pedido.getIdUsuario(), Usuario.class);
		/*return pedido == null ? 
	...
	// Procura via rest se o id passado no pedido existe no serviço de usuario
		public boolean validaPedido(Venda venda){
			...
			Pedido pedido= restTemplate.getForObject("http://localhost:9090/pedido/"+venda.getIdPedido(), Pedido.class);
			if(pedido != null)
			{	return true;
			}
			return false;
