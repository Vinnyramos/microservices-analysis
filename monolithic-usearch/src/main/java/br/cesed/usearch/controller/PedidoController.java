...
public class PedidoController {
	...
	public String getUser(@PathVariable String id) {
		...
		Pedido pedido = pedidoService.getById(id);
		Usuario usuario = restTemplate1.getForObject("http://localhost:9090/usuario/"+pedido.getIdUsuario(), Usuario.class);
		Produto produto = restTemplate2.getForObject("http://localhost:9090/produto/"+pedido.getIdProduto(), Produto.class);
		/*return pedido == null ? 
				new ResponseEntity<Pedido>(HttpStatus.NOT_FOUND) : 
					new ResponseEntity<Pedido>(pedido, HttpStatus.OK);*/
	...
	public boolean validaUsuario(Pedido pedido){
		...
		Usuario usuario = restTemplate.getForObject("http://localhost:9090/usuario/"+pedido.getIdUsuario(), Usuario.class);
		if(usuario != null)
		{return true;
		}
		return false;
	...
	// Procura via rest se o id passado no pedido existe no serviço de produto
		public boolean validaProduto(Pedido pedido){
			...
			RestTemplate restTemplate = new RestTemplate();
			Produto produto = restTemplate.getForObject("http://localhost:9090/produto/"+pedido.getIdProduto(), Produto.class);
			if(produto != null)
			{return true;
			}
			return false;
